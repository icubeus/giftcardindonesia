<?php
class Icube_GiftCardAccount_Model_Observer
{
    /**
     ...
     * @return Icube_GiftCardAccount_Model_Observer
     */
    public function processOrderPlace(Varien_Event_Observer $observer)
    {
        $order = $observer->getEvent()->getOrder();
        $cards = $this->_getHelper('icube_giftcardaccount')->getCards($order);
        if (is_array($cards)) {
            foreach ($cards as &$card) {
                ...
                Mage::dispatchEvent('icube_giftcardaccount_charge', $args);
                $card['authorized'] = $card['ba'];
            }

            $this->_getHelper('icube_giftcardaccount')->setCards($order, $cards);
        }
    ...
    public function processOrderCreateBefore(Varien_Event_Observer $observer)
    {
        ...
        $cards = $this->_getHelper('icube_giftcardaccount')->getCards($quote);

        if (is_array($cards)) {
            foreach ($cards as $card) {
                /** @var $giftCardAccount Icube_GiftCardAccount_Model_Giftcardaccount */
                $giftCardAccount = Mage::getSingleton('icube_giftcardaccount/giftcardaccount')->load($card['i']);
                try {
     ...
     * used for event: icube_giftcardaccount_charge_by_code
     *
     * @param Varien_Event_Observer $observer
     * @return Icube_GiftCardAccount_Model_Observer
     */
    public function chargeByCode(Varien_Event_Observer $observer)
    {
        ...
        $this->_getModel('icube_giftcardaccount/giftcardaccount')
            ->loadByCode($id)
            ->charge($amount)
     ...
     * used for event: icube_giftcardaccount_charge
     *
     * @param Varien_Event_Observer $observer
     * @return Icube_GiftCardAccount_Model_Observer
     */
     ...
     * used for event: icube_giftcardaccount_create
     *
     * @param Varien_Event_Observer $observer
     * @return Icube_GiftCardAccount_Model_Observer
     */
    public function create(Varien_Event_Observer $observer)
    {
        ...
        $model = $this->_getModel('icube_giftcardaccount/giftcardaccount')
            ->setStatus(Icube_GiftCardAccount_Model_Giftcardaccount::STATUS_ENABLED)
     ...
     * used for event: icube_giftcardaccount_save_after
     *
     * @param Varien_Event_Observer $observer
     * @return Icube_GiftCardAccount_Model_Observer
     */
    public function giftcardaccountSaveAfter(Varien_Event_Observer $observer)
    {
        ...
        if ($gca->hasHistoryAction()) {
            $this->_getModel('icube_giftcardaccount/history')
    ...
    public function processOrderCreationData(Varien_Event_Observer $observer)
    {
        ...
        if (isset($request['giftcard_add'])) {
            ...
            try {
                ...
                Mage::getSingleton('adminhtml/session_quote')->addException(
                    $e, Mage::helper('icube_giftcardaccount')->__('Cannot apply Gift Card')
                );
            }
        }
        ...
        if (isset($request['giftcard_remove'])) {
            ...
            try {
                ...
                Mage::getSingleton('adminhtml/session_quote')->addException(
                    $e, Mage::helper('icube_giftcardaccount')->__('Cannot remove Gift Card')
                );
            }
        }
        return $this;
        ...
        if ($paypalCart) {
            ...
            if ($value > 0.0001) {
                $paypalCart->updateTotal(Mage_Paypal_Model_Cart::TOTAL_DISCOUNT, $value,
                    Mage::helper('icube_giftcardaccount')->__('Gift Card (%s)', $this->_getApp()->getStore()->convertPrice($value, true, false))
                );
            }
        }
    }
    ...
    {
        $giftCard = $this->_getModel('icube_giftcardaccount/giftcardaccount')->load($id);
    ...
    public function returnFundsToStoreCredit(Varien_Event_Observer $observer)
    {
        ...
        if (is_array($cards)) {
            ...
            if ($customerId) {
                ...
                if ($balance > 0) {
                    /** @var $balanceModel Icube_CustomerBalance_Model_Balance */
                    $balanceModel = $this->_getModel('icube_customerbalance/balance');
                    $balanceModel->setCustomerId($customerId)
                        ...
                        ->setHistoryAction(Icube_CustomerBalance_Model_Balance_History::ACTION_REVERTED)
                        ->setOrder($order)
                        ->save();
                }
            } else {
