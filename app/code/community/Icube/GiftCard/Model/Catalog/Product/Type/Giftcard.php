<?php
class Icube_GiftCard_Model_Catalog_Product_Type_Giftcard extends Mage_Catalog_Model_Product_Type_Abstract
{
    ...
    {
        if ($this->getProduct($product)->getGiftcardType() == Icube_GiftCard_Model_Giftcard::TYPE_COMBINED) {
            return true;
        }
        return false;
    }
    ...
    {
        if ($this->getProduct($product)->getGiftcardType() == Icube_GiftCard_Model_Giftcard::TYPE_PHYSICAL) {
            return true;
        }
        return false;
    }
    ...
    {
        if ($this->getProduct($product)->getGiftcardType() == Icube_GiftCard_Model_Giftcard::TYPE_VIRTUAL) {
            return true;
        }
        return false;
    }
    ...
    protected function _prepareProduct(Varien_Object $buyRequest, $product, $processMode)
    {
        ...
        try {
            ...
            Mage::logException($e);
            return Mage::helper('icube_giftcard')->__('An error has occurred while preparing Gift Card.');
        }
        ...
        if ($product->getUseConfigAllowMessage()) {
            $messageAllowed = Mage::getStoreConfigFlag(Icube_GiftCard_Model_Giftcard::XML_PATH_ALLOW_MESSAGE);
        } else {
    ...
    private function _validate(Varien_Object $buyRequest, $product, $processMode)
    {
        ...
        if ($emptyFields > 1 && $isStrictProcessMode) {
            Mage::throwException(
                Mage::helper('icube_giftcard')->__('Please specify all the required information.')
            );
        }
        ...
        if (($selectedAmount == 'custom' || !$selectedAmount) && $allowOpen) {
            if ($customAmount <= 0 && $isStrictProcessMode) {
                Mage::throwException(
                    Mage::helper('icube_giftcard')->__('Please specify Gift Card amount.')
                );
            }
            if (!$minAmount || ($minAmount && $customAmount >= $minAmount)) {
                if (!$maxAmount || ($maxAmount && $customAmount <= $maxAmount)) {
                    ...
                    Mage::throwException(
                        Mage::helper('icube_giftcard')->__('Gift Card max amount is %s', $messageAmount)
                    );
                }
            } else if ($customAmount < $minAmount && $isStrictProcessMode) {
                $messageAmount = Mage::helper('core')->currency($minAmount, true, false);
                Mage::throwException(
                    Mage::helper('icube_giftcard')->__('Gift Card min amount is %s', $messageAmount)
                );
            }
        } else if (is_numeric($selectedAmount)) {
            if (in_array($selectedAmount, $allowedAmounts)) {
        ...
        if (!$buyRequest->getGiftcardRecipientName() && $isStrictProcessMode) {
            Mage::throwException(
                Mage::helper('icube_giftcard')->__('Please specify recipient name.')
            );
        }
        if (!$buyRequest->getGiftcardSenderName() && $isStrictProcessMode) {
            Mage::throwException(
                Mage::helper('icube_giftcard')->__('Please specify sender name.')
            );
        }
        ...
        if (!$this->isTypePhysical($product)) {
            if (!$buyRequest->getGiftcardRecipientEmail() && $isStrictProcessMode) {
                Mage::throwException(
                    Mage::helper('icube_giftcard')->__('Please specify recipient email.')
                );
            }
            if (!$buyRequest->getGiftcardSenderEmail() && $isStrictProcessMode) {
                Mage::throwException(
                    Mage::helper('icube_giftcard')->__('Please specify sender email.')
                );
            }
        }
     ...
     *
     * @param Mage_Catalog_Model_Product $product
     * @return Icube_GiftCard_Model_Catalog_Product_Type_Giftcard
     */
