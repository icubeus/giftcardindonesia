<?php
class Icube_GiftCard_Model_Observer extends Mage_Core_Model_Abstract
{
    ...
    public function setAmountsRendererInForm(Varien_Event_Observer $observer)
    {
        ...
        if ($elem) {
            $elem->setRenderer(Mage::app()->getLayout()->createBlock('icube_giftcard/adminhtml_renderer_amount'));
        }
    }
     ...
     *
     * @param Varien_Event_Observer $observer
     * @return Icube_GiftCard_Model_Observer
     */
    public function appendGiftcardAdditionalData(Varien_Event_Observer $observer)
    {
        ...
        if ($product->getUseConfigLifetime()) {
            $lifetime = Mage::getStoreConfig(
                Icube_GiftCard_Model_Giftcard::XML_PATH_LIFETIME,
                $orderItem->getStore()
            );
        } else {
        ...
        if ($product->getUseConfigIsRedeemable()) {
            $isRedeemable = Mage::getStoreConfigFlag(
                Icube_GiftCard_Model_Giftcard::XML_PATH_IS_REDEEMABLE,
                $orderItem->getStore()
            );
        } else {
        ...
        if ($product->getUseConfigEmailTemplate()) {
            $emailTemplate = Mage::getStoreConfig(
                Icube_GiftCard_Model_Giftcard::XML_PATH_EMAIL_TEMPLATE,
                $orderItem->getStore()
            );
        } else {
    ...
    protected function _createGiftCardAccounts(Mage_Sales_Model_Order_Item $item, $numAccounts)
    {
        ...
        for ($i = 0; $i < $numAccounts; $i++) {
            try {
                ...
                Mage::dispatchEvent('icube_giftcardaccount_create', array('request' => $data, 'code' => $code));
    ...
    protected function _sendEmail($item, $numGiftCardsToSend)
    {
        ...
        $codeList = Mage::helper('icube_giftcard')->getEmailGeneratedItemsBlock()
        ...
        $email->sendTransactional(
                ...
                Mage::getStoreConfig(Icube_GiftCard_Model_Giftcard::XML_PATH_EMAIL_IDENTITY, $store->getId()),
    ...
    public function generateGiftCardAccounts(Varien_Event_Observer $observer)
    {
        ...
        $requiredStatus = Mage::getStoreConfig(
            Icube_GiftCard_Model_Giftcard::XML_PATH_ORDER_ITEM_STATUS,
            $order->getStore()
        );

        foreach ($order->getAllItems() as $item) {
            if ($item->getProductType() == Icube_GiftCard_Model_Catalog_Product_Type_Giftcard::TYPE_GIFTCARD) {
                ...
                if ($hasFailedCodes) {
                    ...
                    $message = Mage::helper('icube_giftcard')->__('Some of Gift Card Accounts were not generated properly. You can create Gift Card Accounts manually <a href="%s">here</a>.', $url);
    ...
    public function loadAttributesAfterCollectionLoad(Varien_Event_Observer $observer)
    {
        ...
        foreach ($collection as $item) {
            if (Icube_GiftCard_Model_Catalog_Product_Type_Giftcard::TYPE_GIFTCARD == $item->getTypeId()) {
    ...
    public function initOptionRenderer(Varien_Event_Observer $observer)
    {
        ...
        $block->addOptionsRenderCfg('giftcard', 'icube_giftcard/catalog_product_configuration');
        return $this;
    }
}
