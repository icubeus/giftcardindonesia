 ...
 *
 */
class Icube_CustomerBalance_Model_Observer
{
    /**
     ...
     * @return  Icube_CustomerBalance_Model_Observer
     */
    protected function _checkStoreCreditBalance(Mage_Sales_Model_Order $order)
    {
        if ($order->getBaseCustomerBalanceAmount() > 0) {
            ...
            if (($order->getBaseCustomerBalanceAmount() - $balance) >= 0.0001) {
                ...
                Mage::throwException(
                    Mage::helper('icube_customerbalance')->__('Not enough Store Credit Amount to complete this Order.')
                );
            }
        }
    ...
    public function processOrderPlace(Varien_Event_Observer $observer)
    {
        ...
        if ($order->getBaseCustomerBalanceAmount() > 0) {
            ...
            $websiteId = Mage::app()->getStore($order->getStoreId())->getWebsiteId();
            Mage::getModel('icube_customerbalance/balance')
                ->setCustomerId($order->getCustomerId())
                ->setWebsiteId($websiteId)
                ->setAmountDelta(-$order->getBaseCustomerBalanceAmount())
                ->setHistoryAction(Icube_CustomerBalance_Model_Balance_History::ACTION_USED)
                ->setOrder($order)
                ->save();
        }
    ...
    {
        if (!Mage::helper('icube_customerbalance')->isEnabled()) {
            unset($observer->getUpdates()->icube_customerbalance);
        }
    }
    ...
    public function creditmemoSaveAfter(Varien_Event_Observer $observer)
    {
        ...
        if ($creditmemo->getAutomaticallyCreated()) {
            if (Mage::helper('icube_customerbalance')->isAutoRefundEnabled()) {
        ...
        if ((float)(string)$creditmemo->getCustomerBalanceTotalRefunded() > (float)(string)$customerBalanceReturnMax) {
            Mage::throwException(
                Mage::helper('icube_customerbalance')->__('Store credit amount cannot exceed order amount.')
            );
        }
        ...
        if ($creditmemo->getCustomerBalanceRefundFlag() && $creditmemo->getBsCustomerBalTotalRefunded()) {
            ...
            $balance = Mage::getModel('icube_customerbalance/balance')
                ...
                ->setHistoryAction(Icube_CustomerBalance_Model_Balance_History::ACTION_REFUNDED)
                ->setOrder($order)
    ...
    public function predispatchPrepareLogging($action) {
        ...
        if (isset($data['amount_delta']) && $data['amount_delta'] != '') {
            $actions = Mage::registry('icube_logged_actions');
            if (!is_array($actions)) {
                $actions = array($actions);
            }
            $actions[] = 'adminhtml_customerbalance_save';
            Mage::unregister('icube_logged_actions');
            Mage::register('icube_logged_actions', $actions);
        }
    }
    ...
    {
        Mage::getModel('icube_customerbalance/balance')->setCustomersBalanceCurrencyTo(
        ...
        if ($paypalCart) {
            ...
            if ($value > 0.0001) {
                $paypalCart->updateTotal(Mage_Paypal_Model_Cart::TOTAL_DISCOUNT, (float)$value,
                    Mage::helper('icube_customerbalance')->__('Store Credit (%s)', Mage::app()->getStore()->convertPrice($value, true, false))
                );
            }
        }
    }
